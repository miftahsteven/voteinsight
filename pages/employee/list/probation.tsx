...
import ViewDetailProbation from '../_common/ViewDetailProbation'
...
const Index: NextPage = () => {
	...
	//console.table(' ----< Data Kontrak', dataContractsFinal)
	//console.log(' ----< Dt', data);
		...
		mutatePromote(
			{
				...
				permanent_date: formik.values.permanent_date,
			},
			{
				onSuccess: (data) => {
					if (data) {
	...
	return (
		<PageWrapper>
									...
									<tbody>
										{dataPagination(items, currentPage, perPage).map((i) => (
											<tr key={i.id}>
												...
												<td>{i.division}/{i.group}</td>
												<td>{i.contract_number}</td>
												<td>
													{i.onboarding_date == null
														? 'n/a'
														: i.onboarding_date}
												</td>												
												<td>
			...
			<ViewDetailProbation setIsOpen={setViewDetailDataModal} isOpen={viewDetailDataModal} id={idSelected} dataEmployeeSelected={dataEmployeeSelected} />
			<OffCanvas
			...
			<Modal
				...
				isAnimation={true}>
				<ModalHeader>
					<ModalTitle id='nonactive-user-title'>Promosikan Probation</ModalTitle>
				</ModalHeader>				
						...
						<div className='row g-3'>
							<FormGroup
								...
								<Select
									...
									placeholder='Pilih...'
									list={dataContractsFinal.map((item: any) => ({
										value: item.id,
										text: `${item.contract_number}`,
									}))}
									invalidFeedback={formik.errors.contract_id}
									isTouched={formik.touched.contract_id}
									onFocus={() => {
										formik.setErrors({})
									}}
								/>
							</FormGroup>
							<FormGroup id='permanent_date' label='Tanggal Promosi' className='col-12'>
								<Input
									type='date'
									onChange={formik.handleChange}
									name='permanent_date'
									value={formik.values.permanent_date}
									invalidFeedback={formik.errors.permanent_date}
									isTouched={formik.touched.permanent_date}
									onFocus={() => {
										formik.setErrors({})
									}}
								/>
