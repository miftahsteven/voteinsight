...
const CustomerEditModal: FC<ICustomerEditModalProps> = ({
	...
	if (dataStatus !== undefined) {
		dataStatusRef = dataStatus.data.filter((f:any) => f.id < 8).map((item: any) => ({
			value: item.id,
			text: `${item.name}`,
		}))
	}
									...
									<div className='row g-3'>
										...
										{(dataPIC.length > 0 && interviewId) && (
											<FormGroup
												...
												<Select
													id='status_pic_id'
													ariaLabel='Pilih Interviewer'
													name='status_pic_id'												
													onChange={formik.handleChange}
													value={formik.values.status_pic_id}
													placeholder='Pilih...'
													list={dataPIC}
												/>
											</FormGroup>
